...
def generate_results(min, max, step):
    ...
    while current_size <= max:
        test_array, search_val = generate_array(current_size)
        begin = time.perf_counter()
        binary_search(search_val, test_array, 0, current_size - 1)
        ...
        begin = time.perf_counter()
        linear_search(search_val, test_array)
        end = time.perf_counter()
...
def main():
    ...
    begin = time.perf_counter()
    generate_results(1000000, 10000000, 100000)
    end = time.perf_counter()
